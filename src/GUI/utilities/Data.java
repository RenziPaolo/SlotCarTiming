...
public class Data {
	...
	public int getNumCorsie() {
		if (numLanes==0)
			try(FileChannel fileLanes = (FileChannel) Files.newByteChannel(Path.of("config/settings lanes.config"), StandardOpenOption.READ)){
	...
	public float getMinLapTime() {
		if (minLapTime == 0) {
			try(FileChannel fileMinLapTime = (FileChannel) Files.newByteChannel(Path.of("config/settings minlaptime.config"), StandardOpenOption.READ)){
	...
	public int getPeriod() {
		if (period == 0) {
			try (FileChannel filelanes = (FileChannel) Files.newByteChannel(Path.of("config/settings practice.config"), StandardOpenOption.READ)){
	...
	public String getFont() {
		if (font == null) {
			try {
				font = Files.readString(Path.of("config/settings font.config"));
			} catch (IOException e) {
				System.out.println("eccezione!!");
			}
		}
	...
	public ColorLane getQualifingLane() {
		if (qualifingLane == null) {
			try (FileChannel filequaly = (FileChannel) Files.newByteChannel(Path.of("config/qualifing.config"), StandardOpenOption.READ)){
	...
	public int getMancheduration() {
		if (mancheDuration == 0) {
			try (FileChannel fileMancheDuration = (FileChannel) Files.newByteChannel(Path.of("config/settings MancheDuration.config"), StandardOpenOption.READ)){
				ByteBuffer buffer = ByteBuffer.allocate(8);
	...
	public int[] getRequiredSensor() {
		if (requiredSensor==null) {
			try(FileChannel fileRequiredsensors = (FileChannel) Files.newByteChannel(Path.of("config/settings Requiredsensors.config"), StandardOpenOption.READ)){
	...
	public int[] getAddtionalSensor() {
		if (requiredSensor==null) {
			try(FileChannel fileAdditionalsensors = (FileChannel) Files.newByteChannel(Path.of("config/settings Additionalsensors.config"), StandardOpenOption.READ)){
	...
	public void startRaceLog() {
		
	}
	
	public void addRaceLog(int lane,double lapTime) {
